...
def _require_password(hasher: PasswordHasher, user: User, password: str) -> None:
...
async def change_password(
    ...
    *,
    ...
    _require_password(hasher, user, current_password)
...
async def start_email_change(
    ...
    *,
    ...
    user.email_change_expires_at = datetime.now(UTC) + timedelta(
        seconds=settings.email_change_token_ttl
    )
    await session.flush()
    return raw_token
...
async def confirm_email_change(session: AsyncSession, *, token: str) -> User:
    ...
    if user.email_change_expires_at is None or user.email_change_expires_at < datetime.now(UTC):
...
async def delete_account(
    ...
    *,
    ...
    _require_password(hasher, user, password)
    user_id = user.id
