# Spec 90 — Fix-pack: backend/frontend test & interface gaps (batch 13)
...
Close confirmed test-coverage and documentation gaps around the agent SSE/cancel/
list HTTP endpoints, agent event forward-compat, docs-README link assertions,
docs deliverable-existence guards, the synctex non-member status code, the agent
transcript autoscroll behaviour, and a fetch-stubbed autosave integration test —
without regressions.
