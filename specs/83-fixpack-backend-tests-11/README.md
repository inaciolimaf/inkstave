# Spec 83 — Fix-pack: backend/frontend test & behaviour gaps (batch 11)
...
Close confirmed test-coverage and behaviour gaps across the notifications bell
(true optimistic dismiss + rollback), agent-tools and storage unit tests, the
idle remote-cursor fade, project types, and the Vitest pool config — without
regressions.
