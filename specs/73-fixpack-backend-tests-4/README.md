# Spec 73 — Fix-Pack: Backend & Tests 4 (validated issues)
...
4. **Follow `CLAUDE.md`.** No Overleaf code copied; the stack is fixed
   ...
   Testing Library on the frontend). Match existing style and test patterns.
5. **Test.** After applying the fixes, run the backend and frontend test suites.
   ...
   (xdist) to confirm the budget. Where issues ask for **deterministic** waits,
   prefer the existing `_poll` helper over fixed `asyncio.sleep`.

This pack is mostly test-coverage and type-contract hardening for the collab
realtime core, history UI, editor/file-tree types, and an agent tool. There is
no critical/major item.
...
Close 10 validated collab-compaction, determinism, type-contract, a11y, and
agent-tool coverage gaps from specs 03/17/18/28/30/38/42 without changing public
behaviour beyond the documented fixes.
...
- Do not let the suite exceed the 2-minute budget; deterministic waits over
