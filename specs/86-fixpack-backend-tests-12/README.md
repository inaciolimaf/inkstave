# Spec 86 — Fix-pack: backend/frontend test & changelog gaps (batch 12)
...
Close confirmed test-coverage and changelog gaps across observability
(user_id propagation, Redis-down fakes, /readyz recovery, route-template
cardinality, test log level), the projects N+1 query-count suite, the file-tree
upload-conflict test, the history detail-header label badges, and the spec-40
refactor changelog — without regressions.
